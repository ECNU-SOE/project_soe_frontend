...
2. 实现数据类以及数据解析
    ...
    ```
    - 根据json数据, 接口http://47.101.58.72:8888/corpus-server/api/mistake/v1/getDetail?oneWeekKey=0 返回的数据是```int mistakeTotalNumber,int stubbornMistakeNumber, List eachMistakeTypeNumber,``` , 其中eachMistakeTypeNumber的数据结构为```int mistakeNum, String mistakeTypeName, int mistakeTypeCode```
    - 所以可以定义除DataMistakeListItem和DataMistakeBook
    ```dart
    // 错题本页面的数据
